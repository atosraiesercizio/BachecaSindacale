...
class CRED_Form_Post extends CRED_Form_Base {
	 ...
	 * @return bool|string
	 */
	public function check_redirection( $post_id, $form_id, $form, $_fields, $thisform, $formHelper, $is_ajax, $attachedData ) {
	...
	public function set_authordata() {
		...
		if ( is_int( $this->_post_id ) && $this->_post_id > 0 ) {
			if ( ! isset( $post->ID ) || ( isset( $post->ID ) && $post->ID != $this->_post_id ) ) {
				...
				if ( is_object( $post ) && property_exists( $post, 'post_author' ) ) {
	...
	public function create_new_post( $form_id, $form_type, $post_id, $post_type ) {
		...
		// get post inputs
		if ( isset($post->ID) && $form_type == "edit" ) {
			$post_id = $post->ID;
