...
/**
 * This file is loaded (stand-alone) when performing AJAX file upload from the front-end.
 *
 * Legacy implementation, to be refactored.
 */

define("CRED_WP_REF_FILE", "wp-blog-header.php");

function cred_found_abspath($dir, $file2search) {
    ...
    return cred_found_abspath(dirname($dir), $file2search);
}
...
function cred_get_root_path() {
    return cred_found_abspath(dirname(__FILE__), CRED_WP_REF_FILE);
}
        ...
        if (isset($_GET['id'])) {
            ...
            if (!empty($_FILES)) {
                ...
                foreach ($_FILES as $uploaded_file) {
					...
					// Note that $uploaded_file_tmp_name can be an array on repeating file fields
					if ( is_array( $uploaded_file_tmp_name ) ) {
						foreach ( $uploaded_file_tmp_name as $uploaded_file_tmp_name_repetition ) {
							if (filesize($uploaded_file_tmp_name_repetition) > wp_max_upload_size()) {
						...
						if (filesize($uploaded_file_tmp_name) > wp_max_upload_size()) {
							$data = array('result' => false, 'error' => __('Error: Files is too big, Max upload size is', 'wpv-views') . ': ' . number_format((wp_max_upload_size() / 1048576), 2) . " MB");
							break;
						}
					}
