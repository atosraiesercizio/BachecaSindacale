...
final class CRED_Forms_Model extends CRED_Abstract_Model implements CRED_Singleton {
    ...
    public function getFormsCount($src = '') {
        $sql_add = (isset( $src ) && !empty( $src )) ? $this->wpdb->prepare( " AND (p.post_name like %s || p.post_title like %s) ", '%' . $src . '%', '%' . $src . '%' ) : "";
        ...
        $sql = $this->wpdb->prepare( '
            ...
            WHERE 
                ...
                p.post_type = %s 
                AND 
                p.post_status="private"', $this->prefix . "form_settings", $this->post_type_name );
        $sql .= $sql_add;
    ...
    public function getFormsForTable($page, $perpage, $orderby = 'post_title', $order = 'asc', $src = '') {
        ...
        WHERE 
            ...
            p.post_status='private'", $this->prefix . "form_settings", $this->post_type_name );
        $sql .= $sql_add;
        $sql .= "ORDER BY p.{$orderby} {$order} {$limit}";
    ...
    public function getFormsForExport($ids) {
        ...
        if ( 'all' != $ids ) {
            ...
            WHERE 
                ...
                AND p.ID IN (' . $ids . ')
            ORDER BY p.post_date DESC', $this->prefix . 'form_settings', $this->post_type_name );
        } else {
            $form_query = $this->wpdb->prepare( '
            SELECT p.* FROM ' . $this->wpdb->posts . ' as p, ' . $this->wpdb->postmeta . ' as pm 
            WHERE 
        ...
        if ( 'all' != $ids ) {
            $form_postmeta_query = $this->wpdb->prepare( 'SELECT p.ID, pm.meta_key, pm.meta_value FROM ' . $this->wpdb->posts . ' AS p INNER JOIN ' . $this->wpdb->postmeta . ' AS pm ON p.ID=pm.post_id WHERE 
                (p.post_type=%s AND 
                    p.post_status="private"  ANd p.ID IN (' . $ids . ') AND 
                    pm.meta_key IN (' . $meta_keys . '))', $this->post_type_name );
        } else {
            $form_postmeta_query = $this->wpdb->prepare( 'SELECT p.ID, pm.meta_key, pm.meta_value FROM ' . $this->wpdb->posts . ' AS p INNER JOIN ' . $this->wpdb->postmeta . ' AS pm ON p.ID=pm.post_id WHERE 
                    (p.post_type=%s AND 
