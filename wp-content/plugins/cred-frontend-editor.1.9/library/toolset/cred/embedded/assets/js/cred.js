    ...
    var aux = {
            ...
            toggleHighlight: function () {
                var highlight = '0', active = 'switch-html', inactive = 'switch-tmce',
                    $this = jQuery(this), $cont = $this.closest('.wp-editor-wrap'),
                    qts_cont = $cont.find('.quicktags-toolbar'),
                    qts = $cont.find('.ed_button:not(.qt-fullscreen)'),
                    textb = $cont.find('.switch-text'), syntaxb = $cont.find('.switch-syntax'),
                    switches = $cont.find('.wp-switch-editor')
                ;
            ...
            toggleCodeMirror: function (on) {
                //Remove CRED Button from Editor
                if (window.hasOwnProperty("typenow") && window.typenow == "cred-form") {
                    jQuery("#content").parent().siblings("#wp-content-editor-tools").children("#wp-content-media-buttons").children(".cred-form-shortcode-button2").remove()
                }
                ...
                if (window.CodeMirror) {
                    if (!on && CodeMirrorEditors['content']) {
                        ...
                        jQuery('#content').height('300px').css('padding', '10px');
                        return !on;
                        ...
                        _.defer(function () {
                            ...
                            Toolset.hooks.doAction('toolset_text_editor_CodeMirror_init', 'content');
                        });
                ...
                if (field) {
                    ...
                    if ('checkbox' != field.type) {
                        if (WPML) {
                            ...
                            field_out.push(pad + PAD + '<label>' + field.name + '</label>');
                        }
                    }
                    var args = [field];
        ...
        forms: function (useCodeMirror) {
            ...
            _credView = new mvc.View('cred', _credModel, {
                init: function () {
                        ...
                        // fields can have, placeholder, readonly and escape attributes
                            ...
                            jQuery('#cred_parent_field_settings #cred_parent_required').unbind('change').bind('change', function () {
                                if (jQuery(this).is(':checked')) {
                                    ...
                                    jQuery('#cred_parent_field_settings #cred_parent_no_parent_container').stop(true).slideFadeUp('fast');
                                } else {
                                    jQuery('#cred_parent_field_settings #cred_parent_select_text_container').stop(true).slideFadeUp('fast');
                                    jQuery('#cred_parent_field_settings #cred_parent_no_parent_container').stop(true).slideFadeDown('fast');
                                }
                            });

                            // set default values
                            jQuery('#cred_parent_select_text').val('--- Select ' + data.data.post_type + ' ---');
                            jQuery('#cred_parent_validation_text').val(data.data.post_type + ' must be selected');
                            jQuery('#cred_parent_no_parent_text').val('No Parent');
                            ...
                            jQuery('#cred_parent_extra_button').unbind('click').click(function (event) {
                                ...
                                var no_parent_text = jQuery('#cred_parent_no_parent_text').val();
                    ...
                    //adding quick tags to content editor
                    setTimeout(function () {
                        if (window.QTags && window.QTags.instances.hasOwnProperty(0) && window.QTags.instances[0].id == "content") {
                            WPV_Toolset.add_qt_editor_buttons(window.QTags.instances[0], CodeMirrorEditors["content"]);
                        }
                    }, 200);
            ...
            var text_button = utils.addEditorSwitchButton(cred_settings.locale.text_button_title).addClass('switch-text').addClass("hidden").click(aux.toggleHighlight);
            var syntax_button = utils.addEditorSwitchButton(cred_settings.locale.syntax_button_title).addClass('switch-syntax').addClass("hidden").click(aux.toggleHighlight);

            utils.waitUntilElement('#ed_toolbar .ed_button', function () {
                syntax_button.trigger('click');
                aux.makeResizable(jQuery('#content'));
            });
