...
class CRED_PostExpiration {
			...
			foreach ( $posts_for_notifications as $post_meta ) {
				...
				foreach ( $post_meta->notifications as $key => $notification ) {
					...
					if ( $notification_time <= $now ) {
						...
						add_filter( 'cred_subject_notification_codes', array(
							&$this,
							'extraSubjectNotificationCodes'
						), 5, 3 );
						add_filter( 'cred_body_notification_codes', array(
							&$this,
							'extraBodyNotificationCodes'
						), 5, 3 );
						...
						remove_filter( 'cred_subject_notification_codes', array(
							&$this,
							'extraSubjectNotificationCodes'
						), 5, 3 );
						remove_filter( 'cred_body_notification_codes', array(
							&$this,
							'extraBodyNotificationCodes'
						), 5, 3 );
	...
	public function cred_pe_post_save( $post_id ) {
		...
		if ( $this->_post_expiration_enabled && isset( $_POST['cred_pe'][ $this->_post_expiration_time_field ]['date'] ) && ! empty( $_POST['cred_pe'][ $this->_post_expiration_time_field ]['date'] ) ) {
			...
			$expiration_datetime = $this->get_datetime($expiration_time);
			...
			// expire time default is 0, that means no expiration
			update_post_meta( $post_id, $this->_post_expiration_time_field, $expiration_datetime->getTimestamp() );
	...
	public function cred_pe_render_meta_box_content( $post ) {
		...
		if ( empty( $post_expiration_time ) ) {
		    ...
		    $dt = $this->get_datetime($post_expiration_time);

            $values['minutes'] = $dt->format( 'i' );
			$values['hours']   = $dt->format( 'H' );
			$values['date']    = $dt->format( 'F d, Y' );
		}
	...
	private function get_datetime($timestamp) {
		...
		if ( ! $timezone ) {
			$timezone = get_timezone_name($stored_gtm_offset);
		}
	...
	private function get_timezone_name($stored_gtm_offset) {
		...
		foreach ( $abbrarray as $abbreviation ) {
