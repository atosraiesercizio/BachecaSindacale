...
if (isset($_GET['icon'])) {
    ...
    // &shy; prevents wp to remove empty span (tried 50000 other non-working methods)
    $iconCode  = '<span class="' . $icon_main_class . $icon . '" id="icon-test" style="font-size:' . $iconSize . ';color:' . $iconColor . '">&shy;</span>';
} else {
    $icon      = $default_icon;
    $iconSize  = false;
    $iconColor = false;
    // &shy; prevents wp to remove empty span (tried 50000 other non-working methods)
    $iconCode  = '<span class="' . $icon_main_class . $default_icon . '" id="icon-test">&shy;</span>';
}
?>
<!DOCTYPE html>
<html lang="fr">
