...
//CUSTOM Matteo - modificato per renderlo compatibile VC/porto
var $tmceIframe;
$(window.parent.document.body).find('iframe').each(function(i){
    //console.log($(this).contents());
    if($(this).contents().find('.mce-content-body').length && $(this).attr('id') == 'wpb_tinymce_content_ifr'){ //content_ifr
        $tmceIframe = $(this);
        return false;
    }
});
...
function getBootstrapStyles()
{
    ...
    bodyBackground = $tmceIframe.contents().find('.mce-content-body').css('background-color');
    if(typeof bodyBackground == "undefined") bodyBackground = 'rgb(255, 255, 255)';
    previewBackground = bodyBackground;
...
function getCode(selector)
{
    ...
    if($(window.parent.document.body).find('.mce-tinymce-inline')[0]) {
        ...
        elementCode = $tmceIframe.contents().find(selector).clone().wrap('<div>').parent().html();
    }
    return elementCode;
}
